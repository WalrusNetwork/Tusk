...
import network.walrus.utils.bukkit.PlayerBlockChecker;
...
/**
 * Tracks all damage which was caused by gravity.
 *
...
public class SimpleGravityKillTracker extends AbstractTracker {
  ...
  private static final long MAX_KNOCKBACK_TIME = 20;
   ...
   * Constructor.
   *
   * @param plugin to schedule tasks with
   * @param timer to keep track of server ticks
   */
  public SimpleGravityKillTracker(Plugin plugin, TickTimer timer) {
    this.plugin = plugin;
    this.timer = timer;
  }
  ...
  public void clear(@Nonnull World world) {
    ...
    Iterator<LivingEntity> players = this.falls.keySet().iterator();
    while (players.hasNext()) {
      LivingEntity player = players.next();
  ...
  private void playerBecameAirborne(Fall fall, Location where) {
    if (!fall.isFalling
        && !this.isSupported(fall)
        && this.timer.getTicks() - fall.attackTime <= MAX_KNOCKBACK_TIME) {
      fall.isFalling = true;
   ...
   */
  public void playerAttacked(LivingEntity victim, Entity attacker) {
   ...
   * liquid
   */
  public void playerMoved(LivingEntity player, Location to) {
  ...
  public void playerOnOrOffGround(LivingEntity player, boolean onGround) {
    Fall fall = this.falls.get(player);
    if (fall != null) {
      if (onGround) {
      ...
      if (brokenBlock != null && this.timer.getTicks() - brokenBlock.time <= MAX_SPLEEF_TIME) {
        fall =
            new Fall(
                brokenBlock.breaker, Fall.Cause.SPLEEF, player, Fall.From.FLOOR, brokenBlock.time);
  ...
  /**
   * Called when anything happens to a player that should cancel their fall i.e. death, gamemode
   * change, etc.
   */
  public void cancelFall(LivingEntity victim) {
    this.falls.remove(victim);
  }

  /**
   * Get the Fall that caused the given damage to the given player, or null if the damage was not
   * caused by a Fall.
   */
  public Fall getCausingFall(LivingEntity victim, EntityDamageEvent.DamageCause damageCause) {
  ...
  public void transferOwnership(LivingEntity original, LivingEntity newOwner) {
    Fall fall = this.falls.remove(original);
    if (fall != null) {
      fall.victim = newOwner;
      this.falls.put(newOwner, fall);
    }
  }
}
