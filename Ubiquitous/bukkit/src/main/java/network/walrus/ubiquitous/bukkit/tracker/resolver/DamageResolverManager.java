...
import network.walrus.ubiquitous.bukkit.tracker.info.BukkitDamageInfo;
...
/**
 * Manages the available {@link DamageResolver}s and provides a convenience method to resolve damage
 * using a list of resolvers.
 *
 * <p>It is important what order resolvers are called in since a resolver called after another that
 * returns a non-null damage will override the previous damage. Order can be specified when
 * registering by indicating the types of resolvers that should be called first. Note that there is
 * no guarantee of the order outside of what is specified.
 *
...
public interface DamageResolverManager {
  ...
  /**
   * Registers the specified resolver indicating that certain resolvers must be called before the
   * specified one.
   *
   * @param resolver Specified resolver
   ...
   * @throws IllegalArgumentException if the constraints specified by the before classes cannot be
   *     solved
   */
  void register(
      @Nonnull DamageResolver resolver, @Nonnull Class<? extends DamageResolver>... before);

  /**
   * Registers the specified resolver indication that certain resolvers must be called before the
   * specified one.
   *
   * @param resolver Specified resolver
   * @param before Classes of resolvers that must be called before this one
   ...
   */
  void register(
      @Nonnull DamageResolver resolver,
      @Nonnull Collection<Class<? extends DamageResolver>> before);
  ...
  /**
   * Resolves the specified damage event using the set of registered resolvers.
   *
   * <p>If no resolver can resolve the specified event, this method will return a generic {@link
   * BukkitDamageInfo} instance.
   *
