...
public abstract class LocatableObject<R extends PlayerStore, B> {
  ...
  /**
   * Get the locations from which distance can be measured relative to. The shortest measurement
   * will be used.
   */
   ...
   * the given {@link R}.
   */
  public @Nullable DistanceCalculationMetric getDistanceCalculationMetric(R ref) {
   ...
   */
  public @Nullable DistanceCalculationMetric.Type getDistanceCalculationMetricType(R ref) {
    DistanceCalculationMetric metric = getDistanceCalculationMetric(ref);
    return metric == null ? null : metric.type;
  }

  /** @return the {@link R} who is closest to this object, if any have been tracked. */
  public @Nullable R closest() {
    if (this.distanceByRef.isEmpty()) {
      return null;
    }

    return Ordering.natural()
        .onResultOf(Functions.forMap(this.distanceByRef))
        .immutableSortedCopy(distanceByRef.keySet())
        .get(0);
  }

  /**
   * Is distance relevant at the present moment for the given ref? That is, can it be measured and
   * affect the calculation of closeness?
   */
  ...
  /**
   * Get the minimum distance the given ref has been from the object at any time. The given metric
   * determines exactly how this is measured.
   */
   ...
   */
  public @Nullable Integer resetDistance(R ref) {
   ...
   * @return the old distance, if there was one.
   */
  public @Nullable Integer resetDistance(B base) {
  ...
  public TextStyle distanceStyle(R ref, B viewer) {
    return TextStyle.ofColor(ChatColor.GRAY);
  }
  ...
  public String stringifyDistance(@Nullable R ref, B viewer, boolean sub) {
    ...
    if (distance == Double.POSITIVE_INFINITY) {
      ...
      if (sub) {
        text = StringUtils.subScript(String.format("%.1f", distance));
      } else {
        text = String.format("%.1f", distance);
      }
    }

    return distanceStyle(ref, viewer).apply(text).toLegacyText();
  }
}
